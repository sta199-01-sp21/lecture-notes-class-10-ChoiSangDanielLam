...
- What is the probability a randomly selected patient received the placebo?
  ...
  43/84 is the probability that the patient received a placebo
  
- What is the probability a randomly selected patient received the placebo
  and had a marked improvement?
7/84 is the probability 

  
- What is the probability a randomly selected patient received the placebo
  and the treatment?
  Disjoint probability. Two things not possible at the same time. 0%
...
Probability for blue is 3/8, white is 1/8
...
```
Use sample to run a simulation. Each time you do a sample it will be different. 
...
Pretty close to true probability, just off by a couple of decimals. 
Law of large numbers: more and more, closer to the actual probability
...
Not very likely, but also depends on the color of the marble.
...
```{r identify_events}
two_rolls<- two_rolls %>% 
  mutate(first_match = (roll_1*roll_1 == roll_2), second_match = abs(roll_1-roll_2) == 4 )
two_rolls
```

```{r summarize_rolls}
two_rolls %>% 
  summarize(match_1_prob = mean(first_match), match_2_prob = mean(second_match))
```
